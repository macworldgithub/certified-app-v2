...
export default function VehicleReport() {
  ...
  const reports = [
  {
    vin: "QWERT12345POIUY09", // ✅ Works (confirmed from cURL)
    title: "2020 Toyota Camry",
    date: "2024-01-15",
    inspector: "John Smith",
    score: "8.5",
  ...
  {
    vin: "QWERT12345POIUY09", // ⚠️ Using same valid ID for testing
  ...
  const handleDownloadPDF = async (vin) => {
    try {
      ...
      // ✅ Save file to Downloads (Android)
      const filePath = `${RNFS.DownloadDirectoryPath}/inspection_${vin}.pdf`;
          ...
          {reports.map((item, index) => (
            <View key={index} style={tw`bg-white rounded-xl shadow p-4 mb-4`}>
              ...
              <View style={tw`flex-row justify-between mt-3`}>
                <TouchableOpacity
                  style={tw`items-center`}
                  onPress={() => handleDownloadPDF(item.vin)}
                >
                  <AppIcon name="download" size={16} color="#999" />
                  <Text style={tw`text-gray-500 text-xs`}>
